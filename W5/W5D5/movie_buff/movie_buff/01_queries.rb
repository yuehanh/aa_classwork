...
def harrison_ford
  ...
  Movie.joins(:actors).where(actors: {:name => "Harrison Ford"}).where("castings.ord != 1").select(:id, :title)
end
...
Movie.joins(:actors).group("movies.id").order("COUNT(*) DESC").limit(3).select(:id, :title)
end

def directed_by_one_of(them)
  # Consider the following:
  Movie.joins(:director).where(actors: {name: them} ).select(:id, :title)  # Find the id and title of all the movies directed by one of 'them'.
