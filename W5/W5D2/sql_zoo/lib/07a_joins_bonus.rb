...
def expensive_tastes
  ...
  # determine the average price per track.
    execute(<<-SQL)
    SELECT
          style, SUM(counts.price) / SUM(counts.num)
    FROM
        styles
    JOIN 
        (SELECT
        ...
        WHERE
            price IS NOT NULL
        GROUP BY
            albums.asin) AS counts ON counts.asin = styles.album
    GROUP BY
        style
    ORDER BY
        SUM(counts.price) / SUM(counts.num) DESC, style
    LIMIT 5
