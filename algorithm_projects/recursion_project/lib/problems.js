...
function flatten(data) {
  ...
  for (datum of data) {
    if (datum instanceof Array) {
      result = result.concat(flatten(datum));
    } else {
      result.push(datum);
    }
  }
...
function fileFinder(directories, targetFile) {
  ...
  for (let i = 0; i < keys.length; i++) {
    let item = keys[i];
    if (item[0] === "/") {
      if (fileFinder(directories[item], targetFile)) return true;
    } else {
      if (item === targetFile) return true;
    }
  }
  return false;
}
...
function pathFinder(directories, targetFile) {
  ...
  for (let i = 0; i < keys.length; i++) {
    let item = keys[i];
    if (item[0] === "/") {
      result = pathFinder(directories[item], targetFile);
      if (result) {
        return item + result;
      }
    } else {
